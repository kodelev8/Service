...
{
    public class GetKlantByTaxNo : KlantRepositorySpec
    {
        public class AndException : GetKlantByTaxNo
        {
            [Fact]
            public async void ThenException()
            {
                ...
                //Act
                var result = await Subject.HandleAsync(new KlantRepository.GetKlantByTaxNo {TaxNo = "123456789L01"}, CancellationToken.None);
        ...
        public class AndNoDataFound : GetKlantByTaxNo
        {
            [Fact]
        ...
        public class AndDataFound : GetKlantByTaxNo
        {
            [Fact]
            public async void ThenDataFound()
            {
                ...
                result.Value.Werkgevers.Should().BeEquivalentTo(new List<string> {"123456789L01"});
            }
        }
    }
}
