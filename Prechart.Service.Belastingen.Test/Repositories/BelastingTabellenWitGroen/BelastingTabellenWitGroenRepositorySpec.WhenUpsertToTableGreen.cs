...
{
    public class UpsertToTableGreen : BelastingTabellenWitGroenRepositorySpec
    {
        private List<ITaxRecord> taxRecords;
        public IFluentResults<int> Result { get; private set; }

        public class AndUpsertToTableSuccess : UpsertToTableGreen
        {
            public When GetTax => async () => Result = await Subject.HandleAsync(new BelastingTabellenWitGroenRepository.UpsertToTable { TaxType = "groen", TaxTable = taxRecords }, CancellationToken.None);
            ...
            [Fact]
            public void ThenRecordInsertedToGreen()
            {
                var result = Context.Green.Any().Should().Be(true);
            }

            [Fact]
            public void ThenOldRecordIsDeletedToGreen()
            {
                var result = Context.Green.Any(c => c.Id == 1).Should().BeFalse();
            }
        }

        public class AndException : UpsertToTableGreen
        {
            public When GetTax => async () => Result = await Subject.HandleAsync(new BelastingTabellenWitGroenRepository.UpsertToTable { TaxType = "groen", TaxTable = taxRecords }, CancellationToken.None);
