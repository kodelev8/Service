...
{
    public class WhenUpdateProgress : BatchServiceSpec
    {
        private IFluentResults<bool> result;

        public When UpdateProgress => async () => result = await Subject.HandleAsync(new BatchService.UpdateProgress
        {
            BatchId = "123456789",
            CompletedTask = 1,
            TotalTask = 1,
        ...
        {
            public And Exception => () => _repository.HandleAsync(Arg.Any<BatchRepository.UpdateProgress>(), CancellationToken.None)
        ...
        public class AndFalse : WhenUpdateProgress
        {
            public And False => () => _repository.HandleAsync(Arg.Any<BatchRepository.UpdateProgress>(), CancellationToken.None)
        ...
        public class AndTrue : WhenUpdateProgress
        {
            public And False => () => _repository.HandleAsync(Arg.Any<BatchRepository.UpdateProgress>(), CancellationToken.None)
